/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqsact0.c */
/********************************************************************/
/*                                                                  */
  ...
  for (counter=1; (counter < argc) && !InvalidArgument; counter++)
  {
    ...
    case 'a': /* Application name */
      ...
      else
      {
        ...
        for (i=0; pval[i] != '\0'; i++)    /* Map '/' chars to '&'  */
        {
          if (pval[i] == '/') pval[i] = '&';
        }
...
struct MonDefinitions MonitoringByteStringFields[] =
{
  ...
  {MQBACF_SUB_CORREL_ID,     "SUB_CORREL_ID", NULL},
  {MQBACF_MQSTS_STRUCT ,     "MQSTS Structure", NULL},
  {MQBACF_MQSD_STRUCT  ,     "MQSD Structure", NULL},
  {MQBACF_XA_XID       ,     "XA_XID", NULL},
  ...
  {MQBACF_PROPERTIES_DATA,   "Properties Data", NULL},
};
...
struct MonDefinitions MonitoringIntegerFields[] =
{
  ...
  {MQIACF_NUM_PUBS,           "MQIACF_NUM_PUBS",NULL},
  {MQIACF_HSUB,               "MQIACF_HSUB",NULL},
  {MQIACF_SUBRQ_OPTIONS,      "MQIACF_SUBRQ_OPTIONS",NULL},
  {MQIACF_SUB_OPTIONS,        "MQIACF_SUB_OPTIONS",NULL},
  ...
  {MQIACF_XA_HANDLE,          "MQIACF_XA_HANDLE",NULL},
  {MQIACF_XA_RETVAL,          "MQIACF_XA_RETVAL",NULL},
  {MQIACF_STATUS_TYPE,        "MQIACF_STATUS_TYPE",NULL},
  {MQIACF_XA_COUNT,           "MQIACF_XA_COUNT",NULL},
...
struct MonDefinitions MonitoringStringFields[] =
{
  ...
  {MQCACH_REMOTE_PRODUCT,         "Remote Product", NULL},
  {MQCACH_REMOTE_VERSION,         "Remote Version", NULL},
...
{
  {MQGACF_ACTIVITY_TRACE,  "MQI Operation", NULL},
  {MQGACF_APP_DIST_LIST,   "Distribution List", NULL}
};
...
void printMonInteger(char * _indent, MQLONG _parm, MQINT32 _value)
{
  ...
  for (i=0; (i < (int)(sizeof(MonitoringIntegerFields) /
...
void printMonInteger64(char * _indent, MQLONG _parm, MQINT64 _value)
{
  int i;
  int tagId=-1;
  for (i=0; (i < (int)(sizeof(MonitoringInteger64Fields) /
       sizeof(struct MonDefinitions))) && (tagId == -1); i++)
  {
...
void printMonString(char * _indent, MQLONG _parm, int _len, char * _string)
{
  ...
  for (i=0; (i < (int)(sizeof(MonitoringStringFields) /
       sizeof(struct MonDefinitions))) && (tagId == -1); i++)
  {
  ...
  /* Don't print trailing blank/nulls from the string */
  for (i=_len-1;i>=0;i--)
  {
    if (_string[i] == '\0' || _string[i] == ' ')
    {
      length--;
    }
    else
...
void printMonByteString(char * _indent, MQLONG _parm, int _len, MQBYTE * _array)
{
  ...
  int tagId=-1;
  MQCHAR hexline[80];
  for (i=0; (i < (int)(sizeof(MonitoringByteStringFields) /
       sizeof(struct MonDefinitions))) && (tagId == -1); i++)
  {
                          ...
                          MQLONG Verbose)
{
  ...
  for (item_count=0; (frc == 0) && (item_count < parameterCount); item_count++)
  {
    ...
    /****************************************************************/
    if (bytesLeft < (MQLONG)(sizeof(MQLONG) * 3))
    {
      fprintf(stderr,
                        ...
                        MQLONG *pbuflen)
{
  ...
  MQCHAR CompCodeStr[128];             /* Completion Code            */
  ...
  for (count=0; (frc == 0) && (count < parameterCount); count++)
  {
    ...
    else
    {
      ...
      case MQIACF_COMP_CODE:
        memcpy(&Value32, (ptr + offsetof(MQCFIN, Value)), sizeof(MQLONG));
        strcpy(CompCodeStr, getMonInteger(Parameter,
                                          Value32, buffer, sizeof(buffer)));
        bytesLeft-=Length;
        ptr+=Length;
        break;
