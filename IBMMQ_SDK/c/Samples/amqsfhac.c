 /* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqsfhac.c */
 /********************************************************************/
 /*                                                                  */
...
static int checkReason(char *MQIcall, MQLONG CompCode, MQLONG Reason)
{
  ...
  {
    printf("%s failed with reason code %d\n", MQIcall, Reason);
    wasError = TRUE;
  }
  else if (verbose)
    printf("%s rc = %d\n", MQIcall, Reason);
...
int main( int argc, char** argv )
{
  ...
  char     qmname[MQ_Q_MGR_NAME_LENGTH+1]; /* q manager name         */
  char     qname[MQ_Q_NAME_LENGTH+1];    /* queue name               */
  char     sidename[MQ_Q_NAME_LENGTH+1]; /* side queue name          */
