 /* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqsmhac.c */
 /********************************************************************/
 /*                                                                  */
                        ...
                        MQCBC   * pContext)
 {
   ...
   switch(pContext->Reason)
   {
     ...
     case MQRC_RECONNECTING:
          MQSTAT(hConn, MQSTAT_TYPE_RECONNECTION, &sts, &CompCode, &Reason);

          if ((CompCode == MQCC_OK) && (Reason == MQRC_NONE))
          {
            if ((sts.ObjectQMgrName[0] != '\0') && (sts.ObjectQMgrName[0] != ' '))
              printf("%sEVENT : Connection Reconnecting (Reason: %d, Qmgr:'%.*s', Delay: %dms)\n",TimeStamp,
                     sts.Reason, MQ_Q_MGR_NAME_LENGTH, sts.ObjectQMgrName, pContext->ReconnectDelay);
            else
              printf("%sEVENT : Connection Reconnecting (Reason: %d, Delay: %dms)\n",TimeStamp,
                     sts.Reason, pContext->ReconnectDelay);
          }
          else
 ...
 int main(int argc, char **argv)
 {
   ...
   cbd.CallbackFunction = (MQPTR)EventHandler;
