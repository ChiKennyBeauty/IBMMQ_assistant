/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqsblst.c */
/******************************************************************************/
...
/* beginning and end of the execution cycle. For every N messages             */
...
/* Alternatively the commandline argument can specify a comma separated list  */
/* of message sizes to be used. The sizes in the list will be used cyclically.*/
/*                                                                            */
...
unsigned int  gMessageSizesCount = 0;        /* number of message sizes in list*/
unsigned int *gMessageSizes      = NULL;     /* list of message sizes          */
...
unsigned int  gReportFreq        = 100;      /* frequency of message reporting */
#if MQAT_DEFAULT == MQAT_WINDOWS_NT
...
#define MAX_RANDOM_SIZE 262144               /* maximum permitted message size */
                       ...
                       )
{
  ...
  switch(CompCode) {
    case MQCC_OK:
      vinfo("message received <%s> CorrelID <%s> length=%d (buffer len=%d)"
           , buffer
           , MsgDesc.CorrelId
           , messlen
           , buflen
...
/*                                                                  */
/*   arguments:                                                     */
/*      messageSizes:      List of sizes for messages               */
/*      messageSizesCount: Number of messageSizes                   */
/*                                                                  */
/*   return value:                                                  */
/*      unsigned int : reason code from MQGET (0 if success)        */
/*                                                                  */
/********************************************************************/
unsigned int getMessages(unsigned int messageSizes[],
                         unsigned int messageSizesCount)
{
  ...
  if (messageSizesCount == 0) return 1;

  if (gRandom) {
    bufferSize = MAX_RANDOM_SIZE;
  } else {
    unsigned int i;
    bufferSize = 25;
    for (i=0; i<messageSizesCount; i++) {
      if (messageSizes[i] > bufferSize) bufferSize = messageSizes[i];
    }
  }
  bufferSize++;
  ...
  if (!buffer)
  {
     info("failed to malloc buffer of size %d", bufferSize);
     return 8;
  }
  ...
  while(!(ReturnCode || LastMessage)) {
    if (!gRandom) {
      unsigned int index = (counter-1) % messageSizesCount;
      if (messageSizes[index] > 25) {
        bufferSize = messageSizes[index] + 1;
      } else {
        bufferSize = 25 + 1;
      }
    }
...
unsigned int putMessages(unsigned int Count,
                         unsigned int messageSizes[],
                         unsigned int messageSizesCount)
{
  ...
  unsigned int bufferSize;          /* size of message buffer */
  char     *buffer = NULL;          /* message buffer */
  ...
  while(!ReturnCode && (counter <= Count)) {
    ...
    sprintf(buffer, "blast message %09u", counter);
    if (counter == Count)
    {
      LastMessage = TRUE;
    ...
    if (gRandom) {
      MessageSize = (rand() % MAX_RANDOM_SIZE + 1);
    } else {
      MessageSize = messageSizes[(counter-1) % messageSizesCount];
    }
    /* message must be big enough to contain sequence number */
    if (MessageSize < strlen(buffer)) MessageSize = (unsigned int)strlen(buffer);
    ...
    if (!ReturnCode) {
      vinfo("successfully put msg (%d) size(%d) to queue <%s>",
            counter, MessageSize, gQueueName);
      if (gPutCount % gReportFreq == 0) {
        info("%d messages sent", gPutCount);
      }
    } else {
      info("failed to put msg (%d) size(%d) to queue <%s>",
           counter, MessageSize, gQueueName);
    }
    if (  (gSyncpoint)
       && (  (counter % gUowSize == 0)
...
void usage(void)
{
  printf("Usage: amqsblst queue_manager queue -R -W -r report_freq -s message_size -c message_count -C correlid -v\n");
  printf("    where\n");
  ...
  printf("      -r report_freq   : reporting frequency (default every 100 messages)\n");
  printf("      -s message_sizes : comma separated list of sizes for messages to write, or random (range 1-%d)\n", MAX_RANDOM_SIZE);
...
/********************************************************************/
/* Define a list of valid arguments, it is not valid to specify an  */
/* argument more than once.                                         */
/********************************************************************/
#define      blstValidArgs     "RWuvscCr"
#define      blstArgRead       0
#define      blstArgWrite      1
#define      blstArgUOW        2
#define      blstArgVerbose    3
#define      blstArgMsgSize    4
#define      blstArgMsgCount   5
#define      blstArgCorrelID   6
#define      blstArgReportFreq 7
#define      blstArgLast       7
...
{
  int          Count       = 1;
  char         ValidArgs[] = blstValidArgs;
  BOOLEAN      ArgSeen[blstArgLast+1] = { FALSE };
  char *       p            = NULL;
  char         c            = ' ';
  ...
  if (argc < 4) {
    ...
    while ( !ReturnVal && (Count < argc)) {
      ...
      if (!ReturnVal) {
        ...
        case 'v':
          /* verbose messages */
          if (ArgSeen[blstArgVerbose] == FALSE) {
            gVerboseMode = TRUE;
            ArgSeen[blstArgVerbose] = TRUE;
          } else {
            info("-v already specified");
            ReturnVal = FAIL;
          }
          break;
        ...
        case 'C':
          /* correlID */
          if (ArgSeen[blstArgCorrelID] == FALSE) {
            /* skip to next argument which should be correlID string */
            ++Count;
            ArgSeen[blstArgCorrelID] = TRUE;
            if (Count < argc) {
              vinfo("arg[%d] = <%s>\n\n", Count, argv[Count] );
              strncpy(gCorrelid,argv[Count],sizeof(gCorrelid)-1);
            } else {
              info("command line error, correlid value is missing");
              ReturnVal = FAIL;
            }
          } else {
            info("-C already specified");
            ReturnVal = FAIL;
          }
          break;
        ...
        case 'R':
          /* READ mode */
          if (ArgSeen[blstArgRead] == FALSE) {
            if (gMode == UNITITIALIZED_MODE) {
              gMode = INPUT_MODE;
              ArgSeen[blstArgRead] = TRUE;
            } else {
        ...
        case 'W':
          /* WRITE mode */
          if (ArgSeen[blstArgWrite] == FALSE) {
            if (gMode == UNITITIALIZED_MODE) {
              gMode = OUTPUT_MODE;
              ArgSeen[blstArgWrite] = TRUE;
            } else {
              /* error mode already set */
              info("command line error, options -W and -R are mutually exclusive");
              ReturnVal = FAIL;
        ...
        case 's':
          /* MSGSIZES */
          if (ArgSeen[blstArgMsgSize] == FALSE) {
            /* skip to next argument which should be message sizes */
            ++Count;
            ArgSeen[blstArgMsgSize] = TRUE;
            if (Count < argc) {
              if (NULL != strstr(argv[Count],"random")) {
                ...
                char * pSize;
                int i;
                if ((argv[Count][0] == ',') || argv[Count][strlen(argv[Count])-1] == ',') {
                  info("command line error, message sizes <%s> is invalid", argv[Count]);
                  ReturnVal = FAIL;
                  break;
                }

                gMessageSizesCount = 1;
                for (i=0; argv[Count][i]; i++) if (argv[Count][i]==',') gMessageSizesCount++;
                if (gMessageSizes) free(gMessageSizes);
                gMessageSizes = malloc(gMessageSizesCount * sizeof(unsigned int));
                if (!gMessageSizes) {
                  info("failed to malloc buffer for %d message sizes", gMessageSizesCount);
                  ReturnVal = FAIL;
                  break;
                }

                sizes = malloc(strlen(argv[Count])+1);
                if (!sizes) {
                  info("failed to malloc buffer for sizes argument copy");
                  ReturnVal = FAIL;
                  break;
                }
                strcpy(sizes, argv[Count]);
                pSize = strtok(sizes, ",");
                i = 0;
                while ((pSize != NULL) && ((unsigned int)i < gMessageSizesCount)) {
                  unsigned int sizeValue;
                  sizeValue = atoi(pSize);
                  if (sizeValue > 0) {
                    gMessageSizes[i] = (unsigned int)sizeValue;
                    i++;
                  } else {
                    info("command line error, message sizes <%s> is invalid", argv[Count]);
                    ReturnVal = FAIL;
              ...
              info("command line error, message sizes value is missing");
              ReturnVal = FAIL;
            }
          } else {
            info("-s already specified");
            ReturnVal = FAIL;
          }
          break;
        ...
        case 'c':
          /* MSGCOUNT */
          if (ArgSeen[blstArgMsgCount] == FALSE) {
            /* skip to next argument which should be message Count */
            ++Count;
            ArgSeen[blstArgMsgCount] = TRUE;
            if (Count < argc) {
              ...
              info("command line error, message count value is missing");
              ReturnVal = FAIL;
            }
          } else {
            info("-c already specified");
        ...
        case 'r':
          /* REPORT FREQ */
          if (ArgSeen[blstArgReportFreq] == FALSE) {
            /* skip to next argument which should be reporting frequency */
            ++Count;
            ArgSeen[blstArgReportFreq] = TRUE;
            if (Count < argc) {
              Size = atoi(argv[Count]);
              if (Size > 0) {
                gReportFreq = (unsigned int)Size;
              } else {
                info("command line error, reporting frequency <%s> is invalid", argv[Count]);
                ReturnVal = FAIL;
              }
            } else {
              info("command line error, reporting frequency is missing");
              ReturnVal = FAIL;
            }
          } else {
            info("-r already specified");
        ...
        case 'u':
          ...
          if (ArgSeen[blstArgUOW] == FALSE) {
            ...
            if (Count < argc) {
              ...
              info("command line error, message uow size value is missing");
              ReturnVal = FAIL;
            }
          } else {
            info("-u already specified");
            ReturnVal = FAIL;
          }
          break;
...

{
  ...
  gMessageSizes = malloc(gMessageSizesCount * sizeof(unsigned int));
  if (!gMessageSizes) {
     info("failed to malloc buffer for message sizes");
     return 8;
  }
  gMessageSizes[0]   = 1024;
  gMessageCount      = 1000;
  gVerboseMode       = FALSE;
  gRandom            = FALSE;
  memcpy( gCorrelid
...

{
   ...
   ReturnCode = setDefaults();
   if (ReturnCode) exit((int)ReturnCode);
   ...
   if (QueueOpened  && Connected) {
     if (gMode == OUTPUT_MODE) {
       /* put messages */
       ReturnCode = putMessages(gMessageCount, gMessageSizes, gMessageSizesCount);
       if (!ReturnCode) {
     ...
     if (gMode == INPUT_MODE) {
       ...
       ReturnCode = getMessages(gMessageSizes, gMessageSizesCount);
       if (!ReturnCode) {
   ...
   if (Connected) {
     ...
     if (!ReturnCode) {
       ...
       info("failed to disconnect from queue manager <%s>", gQMgrName );
