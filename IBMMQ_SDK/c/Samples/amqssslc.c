/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqssslc.c */
 /********************************************************************/
 /*                                                                  */
 ...
 /*                         If no extension is provided, .kdb is     */
 /*                         assumed.                                 */
 /*                                                                  */
 /*      -w                 Indicates that a password for the key    */
 /*                         repository file will be supplied and so  */
 /*                         amqssslc should prompt for it to be      */
 /*                         supplied.                                */
 /*                                                                  */
 /*      -i                 Indicates that a unique initial key was  */
 /*                         used to encrypt the key repository       */
 /*                         password and amqssslc should prompt for  */
 /*                         it to be supplied.                       */
 /*                                                                  */
 ...
 IntListEntry CertValPolicyTable[] = {
   ...
   {"none",     "MQ_CERT_VAL_POLICY_NONE",     MQ_CERT_VAL_POLICY_NONE},
   {NULL,       NULL,                  -99}
};
 ...
 static int ProcessCommandLine(int argc,
                               ...
                               MQLONG  *pSuiteB,
                               char   **pOcspUrl,
                               MQLONG  *pPromptKeyPwd,
                               MQLONG  *pPromptInitKey);
 ...
 int main(int argc, char **argv)
 {
   ...
   MQLONG   promptKeyPwd      = 0;
   char     KeyStorePassword[MQ_SSL_ENCRYP_KEY_REPO_PWD_LEN + 1] = {0};
                                       /* Keystore password          */
   MQLONG   promptInitKey     = 0;
   char     InitialKey[MQ_INITIAL_KEY_LENGTH + 1] = {0};
                                       /* Initial Key                */
   ...
   {
     /*******************************************************************/
     /* Get the key repository password, using masked input if possible */
     /*******************************************************************/
     printf("Enter key repository password: ");
     get_password(KeyStorePassword,sizeof(KeyStorePassword)-1);

     if (strlen(KeyStorePassword) > 0 && KeyStorePassword[strlen(KeyStorePassword) - 1] == '\n')
       KeyStorePassword[strlen(KeyStorePassword) -1] = 0;
     SslConnOptions.KeyRepoPasswordPtr = KeyStorePassword;
     SslConnOptions.KeyRepoPasswordLength = (MQLONG)strlen(SslConnOptions.KeyRepoPasswordPtr);

     /* A version 6 MQSCO supports key repository password */
   ...
   /******************************************************************/
   getAuthInfo(&ConnectOptions, &csp);
#endif

   if(promptInitKey)
   {
     /****************************************************************/
     /* Get the initial key, using masked input if possible          */
     /****************************************************************/
     printf("Enter initial key: ");
     get_password(InitialKey,sizeof(InitialKey)-1);

     if (strlen(InitialKey) > 0 && InitialKey[strlen(InitialKey) - 1] == '\n')
       InitialKey[strlen(InitialKey) -1] = 0;
     csp.InitialKeyPtr = InitialKey;
     csp.InitialKeyLength = (MQLONG)strlen(csp.InitialKeyPtr);

     /* A version 2 MQCSP supports initial key, but use version 3 */
     /* in case we're also passing a token for authentication     */
   ...
   /****************************************************************/
   ConnectOptions.SecurityParmsPtr = &csp;
                               ...
                               MQLONG  *pPromptInitKey)
 {
   ...
   while ((rc == OK) && (argno < argc))
   {
     ...
     if (argv[argno][0] == '-')
     {
       ...
       else if (c == 'w')           /* Prompt for keystore password  */
       {
         *pPromptKeyPwd = 1;
         argno ++;
       }
       else if (c == 'i')                 /* Prompt for initial key  */
       {
         *pPromptInitKey = 1;
         argno ++;
       }
