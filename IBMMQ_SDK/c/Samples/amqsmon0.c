/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqsmon0.c */
/********************************************************************/
/*                                                                  */
...
/*            [ -m qmgr           ]      # QMGR to connect to       */
/*            [ -a                |      # Filter QMGR records      */
/*              -q <queueName>    |      # Filter on queue          */
/*              -c <ChannelName>  |      # Filter on channel        */
/*              -i <connectionId> ]      # Filter on connectionId   */
/*            [ -qm <qmgrName>    ]      # Filter on QMGR name      */
/*            [ -n <inputQueue>   ]      # Queue to read from       */
/*            [ -b                ]      # Browse messages          */
/*            [ -d <maxMsgCount>  ]      # Maximum records count    */
/*            [ -w <interval>     ]      # No msg available timeout */
/*            [ -l <fieldList>    ]      # List of fields to report */
/*            [ -s <startTime>    ]      # Read msgs from this time */
/*            [ -e <endTime>      ]      # Read msgs up to this time*/
/*                                                                  */
/********************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
static int ParseOptions(int     argc,
                        ...
                        char    inputQueueName[MQ_OBJECT_NAME_LENGTH],
                        MQLONG  *pinputQueueSpecified,
                        MQLONG  *pbrowse,
                        ...
                        MQCHAR  filterQmgrName[MQ_Q_MGR_NAME_LENGTH],
                        MQLONG  *pfilterQmgr,
...
int getPCFDateTimeFromMsg(MQBYTE *buffer,
                          ...
                          MQCHAR timeString[MQ_TIME_LENGTH+1]);
...
int main(int argc, char **argv)
{
  ...
  MQBYTE   *buffer = NULL;         /* message buffer                */
  ...
  char     qmgrName[MQ_Q_MGR_NAME_LENGTH]="";
  char     inputQueueName[MQ_OBJECT_NAME_LENGTH]="";
  MQLONG   inputQueueSpecified = FALSE;
  MQLONG   monitoringType;
  MQLONG   filterType;
  MQLONG   filterObject;
  char     filterQmgrName[MQ_Q_MGR_NAME_LENGTH]="";
  MQLONG   filterQmgr = FALSE;
  ...
  MQLONG   RecordNum=0;
  MQLONG   fieldList[MAX_FIELDLIST] = {0};
  ...
  if (prc != 0)
  {
    ...
    fprintf(stderr,
      ...
      "       %*.s    [-qm <qmgr_name>]     # Show only records written by a specific QMGR\n"
      "       %*.s [-t accounting]          # Process accounting queue\n"
      "       %*.s    [-a]                  # Show only MQI accounting\n"
      "       %*.s    [-q [queue_name]]     # Show only queue accounting\n"
      "       %*.s    [-i <connectionId>]   # Show only matching connection\n"
      "       %*.s    [-qm <qmgr_name>]     # Show only records written by a specific QMGR\n"
      "       %*.s [-n <input queue>]       # Read records from a specific queue\n"
  ...
  do
  {
    ...
    do
    {
      ...
      {
        MQBYTE *newbuffer = NULL;
        buflen=messlen;
        newbuffer=(MQBYTE *)realloc(buffer, buflen);
        if (newbuffer == NULL)
        {
          fprintf(stderr,
    ...
    else
    {
      ...
      if ((monitoringType == MQCFT_STATISTICS && pcfh->Type != MQCFT_STATISTICS) ||
          (monitoringType == MQCFT_ACCOUNTING && pcfh->Type != MQCFT_ACCOUNTING))
      {
        continue;
      }
      ...
      else if (filterQmgr && (filterQmgrName[0] != '\0') &&
          (checkPCFStringInMsg(buffer + sizeof(MQCFH),
                               pcfh->ParameterCount,
                               MQCA_Q_MGR_NAME,
      ...
      if (intervalStartTime != -1)
      {
        ...
        if (frc == 0)
        {
          ...
          (void)sscanf(MsgStartDate, "%4d-%2d-%2d",
                       &(startTm.tm_year),
                       &(startTm.tm_mon),
                       &(startTm.tm_mday));
          (void)sscanf(MsgStartTime, "%2d.%2d.%2d",
                       &(startTm.tm_hour),
                       &(startTm.tm_min),
                       &(startTm.tm_sec));
      ...
      if (intervalEndTime != -1)
      {
        ...
        if (frc == 0)
        {
          ...
          (void)sscanf(MsgEndDate, "%4d-%2d-%2d",
                       &(endTm.tm_year),
                       &(endTm.tm_mon),
                       &(endTm.tm_mday));
          (void)sscanf(MsgEndTime, "%2d.%2d.%2d",
                       &(endTm.tm_hour),
                       &(endTm.tm_min),
                       &(endTm.tm_sec));
    ...
    switch (parg[1])
    {
      ...
      case 't': /* Statistics || Accounting */
        ...
        else
        {
          ...
          /* Check if accounting is specified */
          for (i=0; (pval[i] != '\0') &&
          ...
          {
            /* Check if statistics is specified */
            for (i=0; (pval[i] != '\0') &&
      ...
      case 'a': /* Show only MQI Accounting or QMGR statistics */
        ...
        break;
      case 'q':
        /* Decide if it's a queue or queue manager filter */
        if (parg[2] == 'm')
        {
          if (*pfilterQmgr)
          {
          ...
          {
            *pfilterQmgr=TRUE;
            for (p=&(parg[3]); *p==' '; p++) ; /* Strip off spaces        */
            ...
            if ((pval != NULL) && (*pval != '\0'))
            {
              for (i=0; (i < MQ_Q_MGR_NAME_LENGTH) && (pval[i] != '\0'); i++)
              {
                filterQmgrName[i]=pval[i];
              }
            }
            else
            {
              /* To filter by QM name, one must be specified */
              InvalidArgument=counter;
            }
          }
        }
        ...
        {
          /* Show only Queue Accounting or statistics    */
          if (filterSpecified)
          {
            InvalidArgument=counter;
          }
      ...
      case 'b': /* Browse messages only */
        ...
        break;
      case 'n': /* Input queue name */
        if (*pinputQueueSpecified)
        {
          InvalidArgument=counter;
        }
        else
        {
          *pinputQueueSpecified=TRUE;
          memset(inputQueueName, 0, MQ_OBJECT_NAME_LENGTH);
          for (p=&(parg[2]); *p==' '; p++) ; /* Strip off spaces        */
          ...
          for (i=0; (*pval != '\0') && (i <MQ_OBJECT_NAME_LENGTH); i++)
          {
            inputQueueName[i]=*pval++;
          }
        }
        break;
        ...
        else
        {
          ...
          else
          {
            ...
            if (*pval != '\0')
            {
              do
              {
                if (isdigit((unsigned char)*pval) && (j < (sizeof(intBuffer)-1)))
                {
                  intBuffer[j++]=*pval;
                  pval++;
                }
      ...
      case 's': /* The start time of records to process */
        ...
        else
        {
          ...
          {
            (void)sscanf(pval, "%4d-%2d-%2d %2d.%2d.%2d",
                         &(startTime.tm_year),
                         &(startTime.tm_mon),
                         &(startTime.tm_mday),
                         &(startTime.tm_hour),
...
struct MonDefinitions MonitoringStringFields[] =
{
  ...
  {MQCACH_REMOTE_PRODUCT,     "RemoteProduct", NULL},
  {MQCACH_REMOTE_VERSION,     "RemoteVersion", NULL},
...
{
  {MQGACF_Q_ACCOUNTING_DATA,  "QueueAccounting", NULL},
  {MQGACF_Q_STATISTICS_DATA,  "QueueStatistics", NULL},
  {MQGACF_CHL_STATISTICS_DATA,"ChannelStatistics", NULL}
};
...
{                                                                   \
  ...
  for (i=0; (i < (MQLONG)(sizeof(MonitoringIntegerFields) /         \
...
{                                                                   \
  MQLONG i;                                                         \
  MQLONG tagId=-1;                                                  \
  for (i=0; (i < (MQLONG)(sizeof(MonitoringInteger64Fields) /       \
       sizeof(struct MonDefinitions))) && (tagId == -1); i++)       \
  {                                                                 \
...
{                                                                   \
  ...
  MQLONG tagId=-1;                                                  \
  MQLONG length=_len;                                               \
  for (i=0; (i < (MQLONG)(sizeof(MonitoringStringFields) /          \
       sizeof(struct MonDefinitions))) && (tagId == -1); i++)       \
  {                                                                 \
...
{                                                                   \
  MQLONG i, j;                                                      \
  MQLONG tagId=-1;                                                  \
  for (i=0; (i < (MQLONG)(sizeof(MonitoringByteStringFields) /      \
       sizeof(struct MonDefinitions))) && (tagId == -1); i++)       \
  {                                                                 \
                          ...
                          MQLONG *fieldList)
{
  ...
  if (indentCount <= (MQLONG)(sizeof(indent)-1))
  {
  ...
  for (count=0; (frc == 0) && (count < parameterCount); count++)
  {
    ...
    /****************************************************************/
    if (bytesLeft < (MQLONG)(sizeof(MQLONG) + sizeof(MQLONG)))
    {
        ...
        if (checkFieldInList(pcfil64->Parameter, fieldList))
        {
          ...
          if (values64 == NULL)
          {
            fprintf(stderr, "Unable to allocate %d bytes for MQCFIN64 list\n",
                    (MQLONG)sizeof(MQINT64) * pcfil64->Count);
            frc=-1;
          }
          else
          {
                          ...
                          MQCHAR timeString[MQ_TIME_LENGTH+1])
{
  ...
  for (count=0; !(dateFound && timeFound) && (count < parmCount); count++)
  {
    ...
    if (*pType == MQCFT_STRING)
    {
      ...
      {
        memcpy(dateString, pcfst->String, MQ_DATE_LENGTH);
        dateString[MQ_DATE_LENGTH]='\0';
        dateFound=TRUE;
      }
      else if (!timeFound && (pcfst->Parameter == timeParameter))
      {
        memcpy(timeString, pcfst->String, MQ_TIME_LENGTH);
