...
/* Notes:                                                                     */
/*                                                                            */
/*    This program was prepared for use with the Sybase database product.     */
/*                                                                            */
/*    Before this sample can be compiled, the database and table must be      */
/*    created, for example:                                                   */
/*                                                                            */
/*    -- Create a 100MB device for the database's data                        */
/*    disk init name='mqbankdbdata',                     \                    */
/*              physname='/sybase/tmpdevs/mqbankdbdata', \                    */
/*              vdevno=5, size=51200                                          */
/*    go                                                                      */
/*    -- Create a 200MB device for the database's transaction log             */
/*    disk init name='mqbankdblog',                      \                    */
/*              physname='/sybase/tmpdevs/mqbankdblog',  \                    */
/*              vdevno=6, size=102400                                         */
/*    go                                                                      */
/*    -- Check devices exist                                                  */
/*    sp_helpdevice                                                           */
/*    go                                                                      */
/*    -- Create the database using (half of the space on) the devices above   */
/*    create database MQBankDB on mqbankdbdata=50 log on mqbankdblog=100      */
/*    go                                                                      */
...
#include <sybtesql.h>                            /* Sybase esql header        */
...
/******************************************************************************/
/* Define a macro and functions for SQL error handling                        */
/******************************************************************************/
#define SETSQLSTR(SQL_STR)                                                     \
   sprintf(sqlErrStr, "Error: %s at line: %d in file: %s\n",                   \
           SQL_STR, __LINE__, __FILE__);

void SQLErrorHandler(int *rc, char *sqlErrStr)
{
   *rc = sqlca.sqlcode;                          /* set return code           */
   printf("Error: %s\n", sqlErrStr);
   printf("SQL CODE: %d\n", sqlca.sqlcode);

   if (sqlca.sqlerrm.sqlerrml)
      printf("SQL Server Error: %s\n", sqlca.sqlerrm.sqlerrmc);
}

void SQLWarningHandler(int *rc, char *sqlErrStr)
{
   *rc = sqlca.sqlcode;                          /* set return code           */
   printf("Error: %s\n", sqlErrStr);
   printf("SQL CODE: %d\n", sqlca.sqlcode);
...
void SQLNotFoundHandler(int *rc, char *sqlErrStr)
{
   *rc = sqlca.sqlcode;                          /* set return code           */
   printf("Error: %s\n", sqlErrStr);
   printf("SQL CODE: %d\n", sqlca.sqlcode);
...
int main(int argc, char *argv[])                 /* ----- START OF MAIN ----- */
{
   ...
   char sqlErrStr[256];                          /* SQL error string          */
   /***************************************************************************/
   /* MQI structures                                                          */
   /***************************************************************************/
   ...
   BOOL DBConnected=FALSE;                       /* database connected        */
   /***************************************************************************/
   ...
   /***************************************************************************/
   /* Set up the SQL error handling functions                                 */
   /***************************************************************************/
   EXEC SQL WHENEVER SQLERROR CALL SQLErrorHandler(&rc, sqlErrStr);
   EXEC SQL WHENEVER SQLWARNING CALL SQLWarningHandler(&rc, sqlErrStr);
   EXEC SQL WHENEVER NOT FOUND CALL SQLNotFoundHandler(&rc, sqlErrStr);
   ...
   while (rc == OK)
   {
      ...
      if (rc == OK)
      {
         ...
         /*********************************************************************/
         /* Even though MQ has connected us with the MQBEGIN we still have to */
         /* explicitly connect to the database.                               */
         /*********************************************************************/
         if (rc == OK && !DBConnected)
         {
            SETSQLSTR ("SET CONNECTION");
            EXEC SQL SET CONNECTION MQBankDB;
         } /* endif */

         if (rc == OK && !DBConnected)
         {
            DBConnected = TRUE;                  /* reset flag now            */
            SETSQLSTR ("USE MQBankDB");
            EXEC SQL USE MQBankDB;
         } /* endif */
         ...
         if (rc == OK)
         {
            ...
            if (rc == OK)
            {
               ...
               /***************************************************************/
               /* With Sybase it's best to close the cursor, even though the  */
               /* MQCMIT below would implicitly close it. We will ignore any  */
               /* error from the close and keep going regardless.             */
               /***************************************************************/
               SETSQLSTR ("CLOSE CURSOR");
               EXEC SQL CLOSE cur;
               rc = OK;
