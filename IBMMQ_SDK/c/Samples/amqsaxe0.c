/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqsaxe0.c */
 /********************************************************************/
 /*                                                                  */
 ...
 /*           Name=SampleApiExit                                     */
 /*                                                                  */
 /*         Note that on AIX pre-built 64 bit versions of the        */
 /*         module are shipped in /usr/mqm/samp/bin, and that on     */
 /*         other UNIXs pre-built 64 bit versions of the module are  */
 /*         shipped in /opt/mqm/samp/bin. These pre-built modules    */
 /*         can be used directly by 64 bit applications by           */
 /*         specifying                                               */
 /*            Module=/usr/mqm/samp/bin/amqsaxe        (AIX)         */
 /*            Module=/opt/mqm/samp/bin/amqsaxe        (other UNIXs) */
 /*         If the sample exit is to be deployed in both 32 and 64   */
 /*         bit environments then it must be built as described in   */
 /*         the MQ knowledge center and stored in /var/mqm/exits and */
 /*         /var/mqm/exits64 as appropriate.                         */
 /*                                                                  */
 ...
 /*   (ie AIX and Linux) both a non-threaded and a threaded          */
 ...
 /********************************************************************/
 /* Macro to find the min of a pair of numbers                       */
 /********************************************************************/
 #ifndef min
...
size_t getMQCNOLength( PMQCNO pConnectOpts )
{
  size_t cnoLength = 0;

  if (myCheckPointer(pConnectOpts))
  {
    switch (pConnectOpts->Version)
    {
      case 0:
      case MQCNO_VERSION_1:
        cnoLength = MQCNO_LENGTH_1;
        break;
      case MQCNO_VERSION_2:
        cnoLength = MQCNO_LENGTH_2;
        break;
      case MQCNO_VERSION_3:
        cnoLength = MQCNO_LENGTH_3;
        break;
      case MQCNO_VERSION_4:
        cnoLength = MQCNO_LENGTH_4;
        break;
      case MQCNO_VERSION_5:
        cnoLength = MQCNO_LENGTH_5;
        break;
      case MQCNO_VERSION_6:
        cnoLength = MQCNO_LENGTH_6;
        break;
      case MQCNO_VERSION_7:
        cnoLength = MQCNO_LENGTH_7;
        break;
      case MQCNO_VERSION_8:
      default:
        cnoLength = MQCNO_LENGTH_8;
        break;
    }
  }
...
size_t getMQODLength( PMQOD pObjDesc )
{
  size_t mqodLength = 0;

  if (myCheckPointer(pObjDesc))
  {
    switch (pObjDesc->Version)
    {
      case 0:
      case MQOD_VERSION_1:
        mqodLength = MQOD_LENGTH_1;
        break;
      case MQOD_VERSION_2:
        mqodLength = MQOD_LENGTH_2;
        break;
      case MQOD_VERSION_3:
        mqodLength = MQOD_LENGTH_3;
        break;
      case MQOD_VERSION_4:
      default:
        mqodLength = MQOD_LENGTH_4;
        break;
    }
  }
...
size_t getMQMDLength( PMQMD pMsgDesc )
{
  size_t mqmdLength = 0;

  if (myCheckPointer(pMsgDesc))
  {
    switch (pMsgDesc->Version)
    {
      case 0:
      case MQMD_VERSION_1:
        mqmdLength = MQMD_LENGTH_1;
        break;
      case MQMD_VERSION_2:
      default:
        mqmdLength = MQMD_LENGTH_2;
        break;
    }
  }
...
size_t getMQPMOLength( PMQPMO pPutMsgOpts )
{
  size_t pmoLength = 0;

  if (myCheckPointer(pPutMsgOpts))
  {
    switch (pPutMsgOpts->Version)
    {
      case 0:
      case MQPMO_VERSION_1:
        pmoLength = MQPMO_LENGTH_1;
        break;
      case MQPMO_VERSION_2:
        pmoLength = MQPMO_LENGTH_2;
        break;
      case MQPMO_VERSION_3:
      default:
        pmoLength = MQPMO_LENGTH_3;
        break;
    }
  }
...
size_t getMQGMOLength( PMQGMO pGetMsgOpts )
{
  size_t gmoLength = 0;

  if (myCheckPointer(pGetMsgOpts))
  {
    switch (pGetMsgOpts->Version)
    {
      case 0:
      case MQGMO_VERSION_1:
        gmoLength = MQGMO_LENGTH_1;
        break;
      case MQGMO_VERSION_2:
        gmoLength = MQGMO_LENGTH_2;
        break;
      case MQGMO_VERSION_3:
        gmoLength = MQGMO_LENGTH_3;
        break;
      case MQGMO_VERSION_4:
      default:
        gmoLength = MQGMO_LENGTH_4;
        break;
    }
  }
...
size_t getMQBOLength( PMQBO pBeginOptions )
{
  size_t boLength = 0;

  if (myCheckPointer(pBeginOptions))
  {
    switch (pBeginOptions->Version)
    {
      case 0:
      case MQBO_VERSION_1:
      default:
        boLength = MQBO_LENGTH_1;
        break;
    }
  }
...
size_t getMQSROLength( PMQSRO pSubRqOpts )
{
  size_t sroLength = 0;

  if (myCheckPointer(pSubRqOpts))
  {
    switch (pSubRqOpts->Version)
    {
      case 0:
      case MQSRO_VERSION_1:
      default:
        sroLength = MQSRO_LENGTH_1;
        break;
    }
  }
...
size_t getMQCBDLength( PMQCBD pCallbackDesc )
{
  size_t cboLength = 0;

  if (myCheckPointer(pCallbackDesc))
  {
    switch (pCallbackDesc->Version)
    {
      case 0:
      case MQCBD_VERSION_1:
      default:
        cboLength = MQCBD_LENGTH_1;
        break;
    }
  }
...
size_t getMQCBCLength( PMQCBC pMQCBContext )
{
  size_t cbcLength = 0;

  if (myCheckPointer(pMQCBContext))
  {
    switch (pMQCBContext->Version)
    {
      case 0:
      case MQCBC_VERSION_1:
        cbcLength = MQCBC_LENGTH_1;
        break;
      case MQCBC_VERSION_2:
      default:
        cbcLength = MQCBC_LENGTH_2;
        break;
    }
  }
...
size_t getMQCTLOLength( PMQCTLO pControlOpts )
{
  size_t ctloLength = 0;

  if (myCheckPointer(pControlOpts))
  {
    switch (pControlOpts->Version)
    {
      case 0:
      case MQCTLO_VERSION_1:
      default:
        ctloLength = MQCTLO_LENGTH_1;
        break;
    }
  }
...
size_t getMQSDLength( PMQSD pSubDesc )
{
  size_t mqsdLength = 0;

  if (myCheckPointer(pSubDesc))
  {
    switch (pSubDesc->Version)
    {
      case 0:
      case MQSD_VERSION_1:
      default:
        mqsdLength = MQSD_LENGTH_1;
        break;
    }
  }
...
size_t getMQSTSLength( PMQSTS pStatus )
{
  size_t mqstsLength = 0;

  if (myCheckPointer(pStatus))
  {
    switch (pStatus->Version)
    {
      case 0:
      case MQSTS_VERSION_1:
        mqstsLength = MQSTS_LENGTH_1;
        break;
      case MQSTS_VERSION_2:
      default:
        mqstsLength = MQSTS_LENGTH_2;
        break;
    }
  }
...
char * strEnvironment ( MQLONG Environment, char *Buffer )
{
  ...
  switch (Environment)
  {
    ...
    default:
      sprintf(Buffer, "%" Int32 "d", Environment);
      p = Buffer;
  }
...
char * strApplType ( MQLONG ApplType, char *Buffer )
{
  ...
  switch (ApplType)
  {
    ...
    case MQAT_AMQP:             p = "MQAT_AMQP";             break;
    default:
      sprintf(Buffer, "%" Int32 "d", ApplType);
      p = Buffer;
  }
...
char * strExitReason ( MQLONG ExitReason, char *Buffer )
{
  ...
  switch (ExitReason)
  {
    ...
    default:
      sprintf(Buffer, "%" Int32 "d", ExitReason);
      p = Buffer;
  }
...
char * strExitResponse ( MQLONG ExitResponse, char *Buffer )
{
  ...
  switch (ExitResponse)
  {
    ...
    default:
      sprintf(Buffer, "%" Int32 "d", ExitResponse);
      p = Buffer;
  }
...
char * strExitResponse2 ( MQLONG ExitResponse2, char *Buffer )
{
  ...
  switch (ExitResponse2)
  {
    ...
    default:
      sprintf(Buffer, "%" Int32 "d", ExitResponse2);
      p = Buffer;
  }
...
char * strAPICallerType ( MQLONG ApiCallerType, char *Buffer )
{
  ...
  switch (ApiCallerType)
  {
    ...
    default:
      sprintf(Buffer, "%" Int32 "d", ApiCallerType);
      p = Buffer;
  }
...
char * strFunctionId ( MQLONG Function, char *Buffer )
{
  ...
  switch (Function)
  {
    ...
    default:
      sprintf(Buffer, "%" Int32 "d", Function);
      p = Buffer;
  }
...
char * strFeedback ( MQLONG Feedback, char *Buffer )
{
  ...
  switch (Feedback)
  {
    ...
    default:
      sprintf(Buffer, "%" Int32 "d", Feedback);
      p = Buffer;
  }
                       ...
                       )
{
  ...
  sprintf(StringBuffer, "%ld.%06ld", counter.tv_sec, (long)(counter.tv_nsec / 1000));
#else
  struct timeval t1;
...
void DumpExitChainArea( FILE * fp, PMQAXP pExitParms )
{
     ...
     )
  {
    ...
    while (ExitChainAreaPtr)
    {
      ...
      {
        fprintf( fp, "    Version:               %" Int32 "d\n",    ExitChainAreaPtr->Version          );
        fprintf( fp, "    StrucLength:           %" Int32 "d\n",    ExitChainAreaPtr->StrucLength      );
        fprintf( fp, "    ChainAreaLength:       %" Int32 "d\n",    ExitChainAreaPtr->ChainAreaLength  );
...
void MQENTRY DumpContext ( FILE *fp, PMQAXC pExitContext )
{
  ...
  fprintf( fp, "   ProcessId:              %" Int32 "d\n", pExitContext->ProcessId );
  fprintf( fp, "   ThreadId:               %" Int32 "d\n", pExitContext->ThreadId );
...
{
  if (pValue) fprintf(fp, "  %14s: (%p) 0x%X\n", name, pValue, (unsigned int)*pValue);
...
{
  if (pValue) fprintf(fp, "  %14s: (%p) %" Int32 "d\n", name, pValue, *pValue);
         else fprintf(fp, "  %14s: (NULL>\n", name);
}
/*********************************************************************/
/*                                                                   */
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            boSize         = getMQBOLength(*ppBeginOptions);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  ...
  DumpHex(fp, *ppBeginOptions, boSize);
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            cnoSize        = getMQCNOLength(*ppConnectOpts);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  ...
  DumpHex(fp, *ppConnectOpts, cnoSize);
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            mdSize         = getMQMDLength(*ppMsgDesc);
  size_t            gmoSize        = getMQGMOLength(*ppGetMsgOpts);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  ...
  DumpHex(fp, *ppGetMsgOpts, gmoSize);
                         ...
                         )
{
  ...
  fprint_hex(fp, "Hconn", pHconn);
  fprint_hex(fp, "Hobj" , pHobj);
  fprintf(fp, "  SelectorCount : %" Int32 "d\n", *pSelectorCount   );
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            mqodSize       = getMQODLength(*ppObjDesc);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            sdSize         = getMQSDLength(*ppSubDesc);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            sroSize        = getMQSROLength(*ppSubRqOpts);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  ...
  fprintf(fp, "  Action        : %" Int32 "d\n", *pAction );
  fprintf(fp, "  ppSubRqOpts   : %s\n", strptr(ppSubRqOpts, "0x%p", buffer1));
  DumpHex(fp, *ppSubRqOpts, sroSize);
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            cbdSize        = getMQCBDLength(*ppCallbackDesc);
  size_t            mdSize         = getMQMDLength(*ppMsgDesc);
  size_t            gmoSize        = getMQGMOLength(*ppGetMsgOpts);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  ...
  fprintf(fp, "  Operation     : %" Int32 "d\n", *pOperation);
  fprint_hex(fp, "Hobj" , pHobj);
  fprintf(fp, "  ppCallbackDesc: %s\n",   strptr(ppCallbackDesc, "0x%p", buffer1));
  DumpHex(fp, *ppCallbackDesc, cbdSize);
                         ...
                         )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            ctloSize        = getMQCTLOLength(*ppControlOpts);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  ...
  DumpHex(fp, *ppControlOpts, ctloSize);
                             ...
                             )
{
  ...
  FILE            * fp             = pExitUserArea->fp;
  size_t            cbcSize        = getMQCBCLength(*ppMQCBContext);
  size_t            mdSize         = getMQMDLength(*ppMsgDesc);
  size_t            gmoSize        = getMQGMOLength(*ppGetMsgOpts);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  ...
  DumpHex(fp, *ppMQCBContext, cbcSize);
  fprintf(fp, "  ppMsgDesc: %s\n",        strptr(ppMsgDesc, "0x%p", buffer1));
  DumpHex(fp, *ppMsgDesc, mdSize);
  fprintf(fp, "  ppGetMsgOpts: %s\n",            strptr(ppGetMsgOpts,     "0x%p", buffer1));
  DumpHex(fp, *ppGetMsgOpts, gmoSize);
                          ...
                          )
{
  ...
  MYEXITUSERAREA  * pExitUserArea  = *ppExitUserArea;
  FILE            * fp             = pExitUserArea->fp;
  size_t            stsSize        = getMQSTSLength(*ppStatus);
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
                         ...
                         )
{
  ...
  char              buffer2[50]    = "";
  size_t            mdSize         = getMQMDLength(*ppMsgDesc);
  size_t            pmoSize        = getMQPMOLength(*ppPutMsgOpts);
                         ...
                         )
{
  ...
  char              buffer1[50]    = "";
  char              buffer2[50]    = "";
  size_t            odSize = getMQODLength(*ppObjDesc);
  size_t            mdSize = getMQMDLength(*ppMsgDesc);
  size_t            pmoSize = getMQPMOLength(*ppPut1MsgOpts);
  ...
  fprint_hex(fp, "Hconn", pHconn);
  fprintf(fp, "  pObjDesc      : %s\n", strptr(ppObjDesc,     "0x%p",   buffer1));
  DumpHex(fp, *ppObjDesc, odSize);
  fprintf(fp, "  MsgDesc       : %s\n", strptr(ppMsgDesc,     "0x%p",   buffer1));
  DumpHex(fp, *ppMsgDesc, mdSize);
  fprintf(fp, "  Put1MsgOpts    : %s\n",strptr(ppPut1MsgOpts, "0x%p",   buffer1));
  DumpHex(fp, *ppPut1MsgOpts, pmoSize);
  fprintf(fp, "  BufferLength  : %s\n", strptr(pBufferLength, "%d",   buffer1));
  fprintf(fp, "  Buffer        : %s\n", strptr(ppBuffer,      "0x%p",   buffer1));
                             ...
                             , PMQLONG   pXARetCode)
{
  ...
  fprintf(fp, "  Rmid          : %" Int32 "d\n",   *pRmid);
  fprintf(fp, "  Flags         : %" Int32 "d\n",   *pFlags);
  return;
}
                          ...
                          , PMQLONG   pXARetCode)
{
  ...
  fprintf(fp, "  Rmid          : %" Int32 "d\n",   *pRmid);
  fprintf(fp, "  Flags         : %" Int32 "d\n",   *pFlags);
  fprintf(fp, "  XA Return code: %" Int32 "d\n",   *pXARetCode);
                                  ...
                                  PMQLONG   pXARetCode)
{
  ...
  fprint_hex(fp, "Hconn", pHconn);
  pValue = *ppHandle;
  fprintf(fp, "  Handle        : %" Int32 "d\n",   *pValue);
  pValue = *ppRetval;
  fprintf(fp, "  Retval        : %" Int32 "d\n",   *pValue);
  fprintf(fp, "  Rmid          : %" Int32 "d\n",   *pRmid);
  fprintf(fp, "  Flags         : %" Int32 "d\n",   *pFlags);
  return;
}
                        ...
                        )
{
  ...
  if (rc == MQRC_NONE)
  {
    ...
    if (env)
    {
      ...
      sprintf( logfile, "%s.%" Int32 "d.%" Int32 "d.log", env, pExitContext->ProcessId, pExitContext->ThreadId );
