/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqsevta.c */
 /********************************************************************/
 /*                                                                  */
 ...
 /*   5724-H72                                                       */
 /*                                                                  */
 /*   (C) Copyright IBM Corp. 2007, 2025 All Rights Reserved.        */
 /*                                                                  */
 ...
 /*     -o Output format (json, text, json_array)                    */
...
OUTPUT_TYPE OutputType = OT_TEXT;
MQBOOL SeenMsg = FALSE;
...
/* processed for escape sequences like '\"' (or worse, \uXXXX format */
/* which could x6 the length in theory).                             */
/* It would be good one day to convert this to                       */
...
#define MAX_GROUP_NESTING 10 /* Max nesting level of groups */
...
{
  struct timeval tv;
  int nonBlocking = -1; /* unknown state */
  fd_set fd;

  do
  {
    /***********************************************************************/
    /* If Unix stdin is not a terminal then try making stdin a nonblocking */
    /* mode rather than fast-spin around the loop. Sleep for a second      */
    /* before looking for the input. This allows programmed timeout on     */
    /* the command even if more messages are being generated. For example  */
    /*      (sleep 5;echo "\n") | amqsevt ...                              */
    /* If the set to nonblocking fails, then continue as before.           */
    /***********************************************************************/
    if (!ISATTY(STDIN_FILENO))
    {
      ...
      {
        int frc;
        frc = fcntl(STDIN_FILENO,F_SETFL,fcntl(STDIN_FILENO, F_GETFL) | O_NONBLOCK);
        if (frc != -1) {
          nonBlocking = 1;
        } else {
          nonBlocking = 0;
                     ...
                     MQCBC   * pContext)
{
  ...
  switch(pContext->CallType)
  {
    ...
    case MQCBCT_MSG_NOT_REMOVED:
      ...
      if (OutputType != OT_TEXT)
        jsonStartEvent();
      if (OutputType == OT_JSON_ARRAY)
      {
        if (SeenMsg)
          jsonAddLine(",");
        else
      ...
      {
        if (jsonCompact == FALSE && (SeenMsg || OutputType == OT_TEXT))
        {
          printf("\n");
        }
        SeenMsg = TRUE;
      ...
      /***********************************************************/
      if (strncmp(pMsgDesc->Format,MQFMT_EMBEDDED_PCF,8) == 0)
      {
        MQEPH *pMQEPH = (MQEPH *) Buffer;
        Buffer = (PMQBYTE) &pMQEPH->PCFHeader;
        Length =  pMQEPH->StrucLength - MQEPH_STRUC_LENGTH_FIXED;
      }
...
int main(int argc, char **argv)
{
  ...
  MQLONG   CReason = MQRC_NONE;    /* reason code for MQCONN        */
    ...
    switch(c)
    {
      ...
      case 'o':
        if (!strcmp(mqoptarg, "json")) {
          OutputType = OT_JSON;
          statusStream = stderr;
        } else if (!strcmp(mqoptarg, "json_compact")) {
          OutputType = OT_JSON;
          statusStream = stderr;
          jsonCompact = TRUE;
        } else if (!strcmp(mqoptarg,"text")) {
          OutputType = OT_TEXT;
        } else if (!strcmp(mqoptarg, "json_array")) {
          OutputType = OT_JSON_ARRAY;
          statusStream = stderr;
        } else {
          error = TRUE;
        }
      ...
      case 'w':
        WaitInterval = atoi(mqoptarg);
        if (WaitInterval > 86400)
          error = TRUE;
        else
          WaitInterval *= 1000; /* Convert seconds to millisec */
        break;
...
MOD_EXIT:
  ...
  if (OutputType == OT_TEXT)
    printf("\nSample AMQSEVT end\n");
  else if (OutputType == OT_JSON_ARRAY && SeenMsg)
  {
    printf("]\n");
    fflush(stdout);
  }
...
static MQBOOL formatEvent(PMQMD pMsgDesc,MQLONG Length,PMQBYTE Buffer)
{
  ...
  int counter;
  int groupLevel = 0;
  int groupCount[MAX_GROUP_NESTING] = {0};
  int totalParameters;
  ...
  if (!error
     ...
     && strncmp(pMsgDesc->Format,MQFMT_EMBEDDED_PCF,8)
  ...
  /* Check the data                                           */
  /************************************************************/
  if (!error && evtmsg->Type > MQCFT_STATUS)
  {
  ...
  {
    lookup(evtmsg->Type,MQCFT_STR,valbuf,sizeof(valbuf));
    if (OutputType != OT_TEXT)
    {
      jsonAddLine("\"msgType\" : {");
      jsonAddLine("    \"name\" : \"%s\",",formatConstant(valbuf));
      jsonAddLine("    \"value\" : %d",evtmsg->Type);
      jsonAddLine("  },");
    }
    else
    {
      sprintf(printbuf,"%s [%d]",formatConstant(valbuf),evtmsg->Type);
      printLine(offset,"Message Type",printbuf);
    }
  }
  ...
  while (counter <= totalParameters)
  {
    ...
    /* group. This can cope with group nesting level up to  */
    /* MAX_GROUP_NESTING.                                   */
    /********************************************************/
    if (groupLevel > 0)
    {
      if (groupCount[groupLevel] == 0)
      {
        if (offset >= 2)
        {
          if (OutputType != OT_TEXT)
    ...
    switch (ElementType)
    {
      case MQCFT_GROUP:
        cfgr = (MQCFGR *)paras;
        groupCount[++groupLevel] = cfgr->ParameterCount;
        totalParameters += groupCount[groupLevel];
      ...
      case MQCFT_INTEGER_LIST:
        ...
        for (i=0;i<cfil->Count;i++)
        {
          ...
          if (groupLevel > 0 && strstr(attrbuf,"_ATTRS"))
          {
            if (cfil->Count == 1 &&
      ...
      case MQCFT_STRING_LIST:
        ...
        if (OutputType != OT_TEXT)
        {
          ...
          /* Print the first item in the list if there is one */
          if (cfsl->Count > 0)
          {
        ...
        else
        {
          if (cfsl->Count > 0)
          {
            /* Print the attribute name and first item in the list */
            printLineNN(offset,formatConstant(attrbuf),
              cfsl->Strings,cfsl->StringLength);
          }
          else
          {
            /* Empty list, so just print the attribute name */
            strcpy(printbuf,"");
            printLine(offset,formatConstant(attrbuf),printbuf);
          }
        }

        /* Print the rest of the items in the list (if any) */
      ...
      case MQCFT_INTEGER:
        ...
        case MQIA_MEDIA_IMAGE_RECOVER_Q:
          fn = MQIMGRCOV_STR;
          break;
        case MQIA_MAX_Q_FILE_SIZE:
          if (cfin->Value == MQQFS_DEFAULT)
            fn = MQQFS_STR;
          else
            sprintf(printbuf,"%d",cfin->Value);
          break;
        ...
        case MQIACF_SUB_OPTIONS:
          ...
          break;
        case MQIACH_SECURITY_PROTOCOL:
          switch(cfin->Value)
          {
            case MQSECPROT_NONE:
              sprintf(printbuf, "%s","PLAINTEXT");
              break;
            case MQSECPROT_SSLV30:
              sprintf(printbuf, "%s","SSLV3");
              break;
            case MQSECPROT_TLSV10:
              sprintf(printbuf, "%s","TLSV1");
              break;
            case MQSECPROT_TLSV12:
              sprintf(printbuf, "%s","TLSV12");
              break;
            case MQSECPROT_TLSV13:
              sprintf(printbuf, "%s","TLSV13");
              break;
            default:
              sprintf(printbuf,"%d",cfin->Value);
              break;
...
/* Buffer size must be at least 2*data length + 1 for NULL               */
/*************************************************************************/
static char *formatHex(PMQBYTE data,char *buf,int datalen)
{
  int i;
...
static char *formatOpenOptions(MQLONG v)
{
  ...
  else
  {
    ...
    if (v & MQOO_BIND_ON_GROUP)
      jstrcat(workBuf,"bind_on_group");
    if (v & MQOO_BROWSE)
      jstrcat(workBuf,"brw");
    if (v & MQOO_CO_OP)
...
static void jsonEndEvent(void)
{
  ...
  if (jsonOutputStart) {
    if (jsonCompact) {
      /*********************************************************************/
      /* Simply replace all NL chars with a space. The normal pretty-print */
      /* has a blank line after each event so this replacement removes it  */
      /* with each event on consecutive lines.                             */
      /*********************************************************************/
      for (i=0;i<(int)strlen(jsonOutputStart);i++) {
        if (jsonOutputStart[i] == '\n')
          jsonOutputStart[i] = ' ';
      }
      printf("%s\n",jsonOutputStart);
    } else {
      /*********************************************************************/
      /* We don't need the \n on this print as it's included in the event  */
      /*********************************************************************/
...
static void jsonAddLine(char *fmt,...)
{
  ...
  if (offset > (jsonOutputSize * 90 /100)) {
    char *newjsonOutputStart = NULL;
    jsonOutputSize = jsonOutputSize*2;
    newjsonOutputStart = realloc(jsonOutputStart,jsonOutputSize);
    if (newjsonOutputStart)
    {
      jsonOutputStart = newjsonOutputStart;
...
static MQBOOL isNumeric(char *v, size_t l)
{
  ...
  if (l == 0)
    return FALSE;
  if (ElementType == MQCFT_STRING || ElementType == MQCFT_STRING_LIST || ElementType == MQCFT_BYTE_STRING)
    return FALSE;
...
static char *jsonEscape(char *s, size_t l)
{
  ...
  for (i=0,j=0;i<l;i++)
  {
    if (isprint((unsigned char)s[i]) || s[i] == 0)
    {
      if (s[i] == '\"')
        jsonEscBuf[j++]='\\';
      if (s[i] == '\\')
      ...
      /*  Convert into the unicode escaped format */
...
static long getEpoch(char *date,char *time)
{
  ...
  memcpy(tmp,date,8);
  (void)sscanf(tmp,"%4d%2d%2d",&y,&m,&d);
  memcpy(tmp,time,8);
  (void)sscanf(tmp,"%2d%2d%2d",&hh,&mm,&ss); /* Ignore hundredths */
...
static char *getTopic(MQHCONN HConn, MQHMSG Hmsg)
{
  char *rc = NULL;
