/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqspse0.c */
/******************************************************************************/
/*                                                                            */
...
/*----------------------------------------------------------------------------*/
  ...
  {                                                                            \
    ...
    GetLocalTime(&sysTime);                                                    \
                                                                               \
    sprintf(pStr, "%.4u-%.2u-%.2u %.2u:%.2u:%.2u.%.3u",                        \
                  sysTime.wYear,                                               \
                  sysTime.wMonth,                                              \
                  sysTime.wDay,                                                \
                  sysTime.wHour,                                               \
...
static MQINT32 CreateOSMutexObj(PMQPSXP pExitParms, char *Name, HMUTEXOBJ* phMutex)
{
  ...
  *phMutex = &mutex;                         /* return the address        */
  ...
  if (rc != OK)
  {
    ...
    GetDateAndTimeToMS(dateAndTimeStr);
    fprintf(pMyExitUserArea->pLogFile,
            "%23.23s  <<<----- Failed to create mutex rc: %u ----->>>\n",
            dateAndTimeStr, rc);
    fflush(pMyExitUserArea->pLogFile);
  } /* endif */
...
static MQINT32 RequestOSMutexObj(PMQPSXP pExitParms, HMUTEXOBJ hMutex)
{
  ...
  if (rc != OK)
  {
    ...
    GetDateAndTimeToMS(dateAndTimeStr);
    fprintf(pMyExitUserArea->pLogFile,
            "%23.23s  <<<----- Failed to get mutex rc: %u ----->>>\n",
            dateAndTimeStr, rc);
    fflush(pMyExitUserArea->pLogFile);
  } /* endif */
...
static MQINT32 ReleaseOSMutexObj(PMQPSXP pExitParms, HMUTEXOBJ hMutex)
{
  ...
  if (rc != OK)
  {
    ...
    GetDateAndTimeToMS(dateAndTimeStr);
    fprintf(pMyExitUserArea->pLogFile,
            "%23.23s  <<<----- Failed to release mutex rc: %u ----->>>\n",
            dateAndTimeStr, rc);
    fflush(pMyExitUserArea->pLogFile);
  } /* endif */
