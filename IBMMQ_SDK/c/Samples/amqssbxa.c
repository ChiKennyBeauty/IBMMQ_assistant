/* @(#) MQMBID sn=p944-L251007.1.DE su=f84fdb91fb9772dfb286437c1f42e7849ffc8dc7 pn=samples/c/amqssbxa.c */
/*********************************************************************/
/*                                                                   */
...
int main(int argc, char **argv)
{
  ...
  MQLONG    waitinterval;                /* Get Wait Interval        */
      ...
      switch(*pFlag)
      {
        ...
        case PARM_FLAG_WAIT_INTERVAL:
          ...
          else
          {
            ...
            {
              waitinterval = ((MQLONG)strtol(ParmWaitInterval, NULL, 0));
...
void printUsage(void)
{
  ...
  printf("                     <seconds>      Numeric wait interval between 0 and 86400\n");
  ...
  for (i = 0; compcode == MQCC_OK; i++)
  {
    ...
    if (compcode != MQCC_OK)
    {
      switch(reason)
      {
        ...
        {
          PMQCHAR newbuffer = NULL;

          /***********************************************************/
          /* The name buffer is too small, reallocate the buffer and */
          /* inquire the same property again.                        */
          /***********************************************************/
          newbuffer = (PMQCHAR)realloc(namebuffer, impo.ReturnedName.VSLength);
    ...
    {
      printf("  %.*s : ", impo.ReturnedName.VSLength, (char*)impo.ReturnedName.VSPtr);
